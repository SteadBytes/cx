...
class CXGeneratorTest(private val name: String, private val tv: CXIdTestVector) {
    @Test
    fun `Contact Identifier iteration sequence`() {
        val gen = when (tv.type as GeneratorType) {
            Type1 -> cxGen1(tv.seed)
            Type2 -> cxGen2(tv.seed)
        }
